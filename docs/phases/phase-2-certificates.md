...
- Route53 hosted zone for `your-domain.com` exists and is authoritative
...
|----------|------|------------|
| `aws_acm_certificate.server` | ACM cert | Domain: `memory.your-domain.com`, validation: DNS |
| `aws_acm_certificate.client` | ACM cert | Domain: `mcp-client.your-domain.com`, validation: DNS, `key_algorithm = "RSA_2048"` |
...
1. Create the client cert via AWS CLI instead:
   ...
   aws acm request-certificate \
     --domain-name mcp-client.your-domain.com \
...
2. Import the resulting cert ARN into Terraform as a data source:
   ...
   data "aws_acm_certificate" "client" {
     domain   = "mcp-client.your-domain.com"
     statuses = ["ISSUED"]
   }
   ```
...
The server cert (`memory.your-domain.com`) is a standard non-exportable ACM cert and is fully supported by Terraform.
...
|----------|------|-------------|
| `domain_name` | `string` | Base domain (default: `"your-domain.com"`) |
...
- **No wildcard certs:** Each cert is for a specific FQDN. No `*.your-domain.com`.
...
3. Create `terraform/modules/certificates/main.tf`:
   - Server cert: `aws_acm_certificate` with `domain_name = "memory.your-domain.com"`, `validation_method = "DNS"`
