...
Creates the API Gateway HTTP API with a custom domain, mTLS authentication, route definitions, and Lambda proxy integration. After this phase, the service is reachable over the internet via `https://memory.your-domain.com` with mTLS enforcement.

## Prerequisites

- Phase 1 complete: S3 bucket with truststore PEM at `mtls/truststore.pem`
- Phase 2 complete: ACM server cert ISSUED for `memory.your-domain.com`
...
| `aws_apigatewayv2_domain_name.memory` | Custom domain | Domain: `memory.your-domain.com`, cert ARN, mTLS truststore |
...
resource "aws_apigatewayv2_domain_name" "memory" {
  domain_name = "memory.your-domain.com"
...
| `custom_domain_url` | `https://memory.your-domain.com` | Phase 5 (MCP server config), Phase 7 (hook script) |
...
aws apigatewayv2 get-domain-name --domain-name memory.your-domain.com \
  --query 'MutualTlsAuthentication.TruststoreUri'
# Expected: s3://engram-memory-.../mtls/truststore.pem

# Verify DNS resolves
dig memory.your-domain.com +short
...
curl --cert /tmp/client.crt --key /tmp/client.key \
  ...
  -X POST https://memory.your-domain.com/store \
...
curl -X POST https://memory.your-domain.com/store \
  -H "Content-Type: application/json" \
...
curl --cert /tmp/client.crt --key /tmp/client.key \
  --cacert /tmp/amazon-trust-services-ca.pem \
  -X POST https://memory.your-domain.com/recall \
  -H "Content-Type: application/json" \
  -d '{"query":"api test"}'
