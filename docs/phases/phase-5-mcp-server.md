...
- Phase 4 complete: API Gateway reachable at `https://memory.your-domain.com` with mTLS
...
```json
{
  "engram-memory": {
    ...
    "cwd": "<home>/MyProjects/engram",
    "env": {
      "MEMORY_API_URL": "https://memory.your-domain.com",
...
7. Test the server starts and lists tools:
   ```bash
   cd <home>/MyProjects/engram
   MEMORY_API_URL=https://memory.your-domain.com python -m mcp_server.server
   ```
